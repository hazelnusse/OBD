...
int main(int argc, char ** argv)
{
  ...
  //gsl_matrix_complex_fprintf(stdout, &(m.matrix), "%0.16f");
  ...
  for (int j = 0; j < 3; ++j) {
    ...
    if (j == 0) {
      assert(fabs(
            ...
            - 3.78290405129320) <= 1e-14);
      assert(fabs(
            GSL_REAL(gsl_vector_complex_get(bb->evals, 2))
            - 3.78290405129320) <= 1e-14);
      assert(fabs(GSL_IMAG(gsl_vector_complex_get(bb->evals, 1)))
            <= 1e-7); // not exactly real
      assert(fabs(GSL_IMAG(gsl_vector_complex_get(bb->evals, 2)))
            <= 1e-7); // not exactly real
    }
    if (j == 1) {
      assert((fabs(
      ...
      assert((fabs(
             ...
             (fabs(
              GSL_IMAG(gsl_vector_complex_get(bb->evals, 2))
              + 3.43503384866144) <= 1e-14));
      assert(fabs(GSL_REAL(gsl_vector_complex_get(bb->evals, 1)))
             <= 1e-14); // not purely imag
      assert(fabs(GSL_REAL(gsl_vector_complex_get(bb->evals, 2)))
             <= 1e-14); // not purely imag
    }
    if (j == 2) {
      assert(fabs(GSL_REAL(gsl_vector_complex_get(bb->evals, 0)))
          <= 1e-14);
      assert(fabs(GSL_IMAG(gsl_vector_complex_get(bb->evals, 0)))
          <= 1e-14);
    }
    cout << "All tests pass.\n";
  } // for j
  ...
  cout << "Table 2(b) & 2(b) verifications\n";
  for (int j = 0; j < 11; ++j) {
    ...
    if (j == 0) {
        ...
        assert((fabs(GSL_REAL(gsl_vector_complex_get(bb->evals, 3)) -
                GSL_REAL(gsl_matrix_complex_get(&(m.matrix), 2, j)))
                <= 1e-14)
               ^
        ...
        assert(signbit(GSL_REAL(gsl_vector_complex_get(bb->evals, 2))) !=
      ...
      for (int i = 0; i < 4; ++i) {
        if (i == 0) {
          assert(fabs(GSL_REAL(gsl_vector_complex_get(bb->evals, i))
                      -
                      GSL_REAL(gsl_matrix_complex_get(&(m.matrix), i, j)))
                      <= 1e-13); // all passed with 1e-13, not 1e-14
        }
        if (i == 1) {
          // double check that they are complex conjugates, probably a redudant
          // test
          // all passed
          assert(GSL_REAL(gsl_vector_complex_get(bb->evals, i)) ==
                 GSL_REAL(gsl_vector_complex_get(bb->evals, i+1)));
          assert(GSL_IMAG(gsl_vector_complex_get(bb->evals, i)) ==
